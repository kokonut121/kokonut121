# patrick feng

cs + neuro @ caltech

researching and developing ai4science & world models at the technical frontier

---
### current projects

| project | description |
|---|---|
| [Annotagent](https://github.com/PitchFork-Innovation/annotagent) | agentic annotation tool for arxiv research papers |
| [MedSim](https://github.com/kokonut121/medsim) | agent swarm simulations on world model reconstructions for trauma center optimization & most innovative @ harvard hsil hackathon |
| [ZombieSlayer](https://github.com/jerryjlwang/ZombieSlayer)| safeguarding agents from zombie prompt injections |
| [WP Film Coach](https://github.com/kokonut121/wp-film-coach) | play-by-play analysis for water polo game film with computer vision powered agents |

---
### research

- neural operators for spatio-temporal modeling and dynamical control of complex biological systems
- sampling algorithms for efficient/robust ebms and world models
- journal published and conference accepted (aiche)
- also interested in autonomous/robotic systems

---
### about me

- published researcher at the university of washington
- director of r&d @ [pitchfork](https://www.usepitchfork.com/#innovation) w/ patent pending
- cto @ [the mu foundation](https://github.com/The-Mu-Foundation)
- ncaa d3 recruited athlete

---
### stack

**languages** `python` `java` `c` `typescript`

**misc** `pytorch` `aws` `node.js` `next.js`

<!--
**kokonut121/kokonut121** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
